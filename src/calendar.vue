  ...
  <div class="m-calendarWrap" v-show="show" transition="fade" v-on:click="hideCalendar()">
    <div class="m-calendar" v-on:click="stopPop($event)">
      ...
      <table class="calendarTable">
        ...
        <tr>
          <th :class="{'z-weekend': $index ==0 || $index == 6}" v-for="(text, $index) in weekTxt">{{text}}</th>
        </tr>
        </thead>
        <tbody class="calendarWrap">
        <tr v-for="week in weeks">
          <td :class="{'z-disabled': day.disabled, 'z-active': day.selected, 'z-today': day.isToday}"
              v-for="day in week"
  ...
  export default{
    ...
    methods: {
      ...
      changeMonth ($event, offset){
        const _date = moment(this.currentDate);
        _date.month(_date.month() + offset);
        this.$emit('update:currentDate', formatDate(_date.format('YYYY-MM-DD')));
        this.handelDate(_date.format('YYYY-MM-DD'))
      },
      selectDay ($event, day) {
        this.$emit('update:currentDate', formatDate(moment(day.date).format('YYYY-MM-DD')));
        this.$emit('update:show', !1);
        this.handelDate(moment(day.date).format('YYYY-MM-DD'))
      },
      manageDate (date) {
      ...
      handelDate (date) {
        this.weeks = this.manageDate(date);
        this.currentDateTxt = moment(date).format('YYYY年MM月')
      },
    },
  }
